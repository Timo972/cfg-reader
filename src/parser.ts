...
export function isFloat(value: string): boolean {
  const x = value.split(".");
  return x.length == 2 && x.every(isInt);
}
...
export class Parser {
  ...
  public parse(): Dict {
    ...
    return this.parseToken() as Dict;
  }
  ...
  protected tokenize(): void {
    ...
    while (this.unread() > 0) {
      ...
      if (this.peek() == "[") {
        ...
        this.skip();
        this.tokens.push(
          new Token(TokenType.DictEnd, "", this.readPos, this.line, this.column)
        );
      } else {
        let val = "";
        ...
        if (this.unread() > 0 && this.peek() == ":") {
          this.tokens.push(
            new Token(TokenType.Key, val, this.readPos, this.line, this.column)
          );
        } else {
          this.tokens.push(
            new Token(
  ...
  protected parseToken(): ConfigValue {
    ...
    switch (token.type) {
      case TokenType.Scalar:
        ...
        if (
          ...
          return token.value === "true" || token.value === "yes";
        } else if (isInt(token.value) || isFloat(token.value)) {
      ...
      case TokenType.ArrayStart:
        const list: List = [];
        while (
          this.tokIdx < this.tokens.length - 1 &&
          this.tokens[this.tokIdx + 1].type != TokenType.ArrayEnd
        ) {
          this.tokIdx++;
          list.push(this.parseToken());
        }
        this.tokIdx++;
      ...
      case TokenType.DictStart:
        ...
        while (
          this.tokIdx < this.tokens.length - 1 &&
          this.tokens[this.tokIdx + 1].type != TokenType.DictEnd
        ) {
          this.tokIdx++;
          const nextTok = this.tokens[this.tokIdx];
          if (nextTok.type != TokenType.Key) {
            throw new Error(
              this.createParseError(ErrorType.KeyExpected, nextTok)
            );
          }

          const key = nextTok.value;
          this.tokIdx++;
          dict[key] = this.parseToken();
        }
        this.tokIdx++;
